...
class Field
  ...
  def layout_cells
	self[0,0] = MIDICell.new(lambda{
	  test_drive_arpeggio(@midi,@root_note,@scale,@chord,@progression,@duration)})
	end
...
Shoes.app :width => 730, :height => 550, :title => 'Minesweeper' do
  def render_field
	clear do
	  ...
	  flow do
		(32..127).each {|n|
		  button Note.name_from_value(n), :width => 40 do
