...
class LiveMIDI
  ...
  def play(channel, note, duration, velocity=100, time=nil)
    ...
    #~ p duration
    sleep(duration)
    off_time = on_time + duration.to_f
...
#~ if RUBY_PLATFORM.include?('mswin')
  class LiveMIDI
    module C
      ...
      #~ extern "int midiOutGetDevCaps(UINT_PTR*, int, int)"
      #~ extern "int mciSendCommand(MCIDEVICEID*, int, int, int)"
    ...
    #~ def get_device_id
      ...
      #~ p C.mciGetDeviceID(@device)
    #~ end

    #~ def play_file
      #~ @device = DL::CPtr.new(DL.malloc(DL::SIZEOF_INT),DL::SIZEOF_INT) 
      #~ C.mciSendCommand(@device, 0, 0, 0)
    #~ end
